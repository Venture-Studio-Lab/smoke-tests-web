...
let sessionId;
const prefix=`vs6c:${env.experimentVersion}:${slug}:`;
...
function addOptions(id,items){const el=document.getElementById(id);(items||[]).forEach(v=>{const o=document.createElement("option");o.value=v;o.textContent=v;el.appendChild(o)})}
function render(){
 document.title=`${cfg.label} — Early access concept`;
 txt("rootBadge",cfg.root);txt("audience",cfg.audience);txt("headline",cfg.headline);txt("subheadline",cfg.subheadline);
 txt("planName",plan.name);txt("planPrice",plan.price);txt("planDetail",plan.detail);txt("roleLabel",cfg.rolePrompt);
 txt("whyTitle",cfg.whyTitle);txt("whyText",cfg.whyText);txt("boundary",cfg.boundary);
 const ft=document.getElementById("freeTierNote");if(cfg.freeTierNote){ft.textContent=cfg.freeTierNote;ft.classList.remove("hide")}
 ...
 addOptions("roleSelect",cfg.roles);addOptions("toolSelect",cfg.tools);
...
form.addEventListener("submit",async e=>{
 ...
 const submit=form.querySelector('button[type="submit"]');submit.disabled=true;status.textContent="Saving reservation…";
 try{
